...
#define status_require if(status_failure_p){status_goto;}
#define status_set_group(group_id) status.group=group_id
#define status_set_id(status_id) status.id=status_id
#define status_set_both(group_id,status_id) status_set_group(group_id);status_set_id(status_id)
/** update status with the result of expression, check for failure and goto error if so */
#define status_require_x(expression) status=expression;if(status_failure_p){status_goto;}
/** set the status id and goto error */
#define status_set_id_goto(status_id) status_set_id(status_id);status_goto
#define status_set_group_goto(group_id) status_set_group(group_id);status_goto
#define status_set_both_goto(group_id,status_id) status_set_both(group_id,status_id);status_goto
...
typedef uint32_t_s status_i_t;typedef struct{status_i_t id;uint8_t group;} status_t;