...
(define
  ...
  (sp-windowed-sinc-bp-br
    in in-size cutoff-l cutoff-h transition-l transition-h is-reject out-state out-samples))
...
(define (sp-cheap-filter-state-new max-size max-passes out-state)
  ...
  (set
    out-state:in-temp in-temp
    out-state:out-temp out-temp)
  (memset out-state:svf-state 0 (* (sizeof sp-sample-t) 2 sp-cheap-filter-passes-limit))
  (label exit
    (if status-is-failure (free in-temp))
    (return status)))
...
(define (sp-cheap-filter type in in-size cutoff passes q-factor unity-gain state out)
  ...
  (if (= 1 passes) (type out in in-size cutoff q-factor state:svf-state)
    (begin
      ...
      (set
        passes (- passes 1)
        in-temp state:in-temp
        out-temp state:out-temp)
      (label loop
        (if (< 1 passes)
          (begin
