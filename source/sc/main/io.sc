(define (sp-file-close file) (s-t sp-file-t*)
  s-declare
  (set s-current.id (sf-close (convert-type file:data SNDFILE*)))
  (if s-is-success (set s-current.group sp-s-group-sndfile)
    (set file:flags (bit-or sp-file-bit-closed file:flags)))
  s-return)

(define (sp-file-open path mode channel-count sample-rate result-file)
  (s-t uint8-t* int sp-channels-t sp-sample-rate-t sp-file-t*)
  s-declare
  ...
  (case = mode
    ...
    (sp-file-mode-read-write
      (set
        ...
        info.channels channel-count
        info.samplerate sample-rate))
    (else (s-set-goto sp-s-group-sp sp-s-id-file-type)))
  (set file (sf-open path sf-mode &info))
  (if (not file) (s-set-goto sp-s-group-sndfile (sf-error file)))
  (set
...
(define (sp-file-write file channel-data sample-count result-sample-count)
  (s-t sp-file-t* sp-sample-t** sp-time-t sp-time-t*)
  ...
  (s (sph-helper-malloc interleaved-size &interleaved))
  ...
  (if (not (= sample-count frames-count)) (s-set sp-s-group-sp sp-s-id-file-incomplete))
...
(define (sp-file-read file sample-count result-channel-data result-sample-count)
  ...
  (if frames-count (sp-deinterleave result-channel-data interleaved frames-count channel-count)
    (s-set sp-s-group-sp sp-s-id-eof))
  (set *result-sample-count frames-count)
  (label exit memreg-free s-return))

(define (sp-file-position-set file a) (s-t sp-file-t* sp-time-t)
  ...
  (if (= count -1) (s-set-goto sp-s-group-sndfile (sf-error snd_file)))
  (label exit s-return))

(define (sp-file-position file result-position) (s-t sp-file-t* sp-time-t*)
  s-declare
  (declare snd_file SNDFILE* count sf-count-t)
