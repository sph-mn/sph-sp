...
(define (test-moving-average) status-t
  ...
  (set
    ...
    in-window-end (+ in 3)
    radius 3)
  (status-require (sp-moving-average in in-end in-window in-window-end prev prev-end next next-end radius out))
...
(define (test-sp-seq) status-t
  ...
  (status-require (sp-synth-event (+ 2 sp-seq-half-duration) (- sp-seq-duration 2) 1 1 config (+ 1 events)))
  (set events-size 2)
...
(define (test-sp-cheap-noise-event) status-t
  ...
  (for ((set i 0) (< i sp-noise-duration) (set i (+ 1 i)))
    (set
      ...
      (array-get amp1 i) 1.0))
  (status-require (sp-cheap-noise-event-lp 0 sp-noise-duration amp cut 1 0 #f sp-default-random-state events))
  (set events-size 1)
  (sp-seq 0 sp-noise-duration out events events-size)
  #;(for ((set i 0) (< i sp-noise-duration) (set i (+ 1 i)))
    (printf "%f " (array-get *out.samples i)))
