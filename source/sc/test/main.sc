...
(define (test-spectral-inversion-ir) s-t
  s-declare
...
(define (test-spectral-reversal-ir) s-t
  s-declare
  (declare a-len sp-time-t a (array sp-sample-t 5 0.1 -0.2 0.3 -0.2 0.1))
  (set a-len 5)
...
(define (test-convolve) s-t
  ...
  (s (sph-helper-calloc (* result-len (sizeof sp-sample-t)) &result))
  (memreg-add result)
  (s (sph-helper-calloc (* a-len (sizeof sp-sample-t)) &a))
  (memreg-add a)
  (s (sph-helper-calloc (* b-len (sizeof sp-sample-t)) &b))
  (memreg-add b)
  (s (sph-helper-calloc (* carryover-len (sizeof sp-sample-t)) &carryover))
...
(define (test-moving-average) s-t
  s-declare
  (declare
    radius sp-time-t
  ...
  (set
    ...
    in-window-end (+ in 3)
    radius 3)
  (s (sp-moving-average in in-end in-window in-window-end prev prev-end next next-end radius out))
  ...
  (s (sp-moving-average in in-end in in-end 0 0 0 0 1 out))
...
(define (test-windowed-sinc) s-t
  ...
  (s (sp-windowed-sinc-lp-hp-ir cutoff transition #f &ir &ir-len))
  (test-helper-assert "ir" (sp-sample-nearly-equal 0.0952 (array-get ir 28) error-margin))
  (s (sp-windowed-sinc-bp-br-ir 0.1 0.4 0.08 0.08 #f &ir &ir-len))
  (s (sp-windowed-sinc-lp-hp-ir cutoff transition #t &ir &ir-len))
  (s (sp-windowed-sinc-bp-br-ir cutoff cutoff transition transition #t &ir &ir-len))
  (sc-comment "filter functions")
  (s (sp-windowed-sinc-lp-hp source 10 0.1 0.08 #f &state result))
  (sp-convolution-filter-state-free state)
  (set state 0)
  (s (sp-windowed-sinc-lp-hp source 10 0.1 0.08 #t &state result))
  (sp-convolution-filter-state-free state)
  (set state 0)
  (s (sp-windowed-sinc-bp-br source 10 0.1 0.4 0.08 0.08 #f &state result))
  (sp-convolution-filter-state-free state)
  (set state 0)
  (s (sp-windowed-sinc-bp-br source 10 0.1 0.4 0.08 0.08 #t &state result))
  (sp-convolution-filter-state-free state)
  (label exit s-return))

(define (test-file) s-t
  s-declare
  (declare
    channel sp-time-t
  ...
  (s (sp-block-new channel-count sample-count &block))
  (s (sp-block-new channel-count sample-count &block-2))
  (while channel
  ...
  (s (sp-file-open test-file-path sp-file-mode-read-write channel-count sample-rate &file))
  (printf "  create\n")
  (s (sp-file-position &file &position))
  (s (sp-file-write &file block.samples sample-count &result-sample-count))
  (s (sp-file-position &file &position))
  (test-helper-assert "sp-file-position file after write" (= sample-count position))
  (s (sp-file-position-set &file 0))
  (s (sp-file-read &file sample-count block-2.samples &result-sample-count))
  ...
  (s (sp-file-close &file))
  (printf "  write\n")
  (sc-comment "test open")
  (s (sp-file-open test-file-path sp-file-mode-read-write 2 8000 &file))
  (s (sp-file-position &file &position))
  ...
  (s (sp-file-close &file))
  (printf "  open\n")
  (label exit (sp-block-free block) (sp-block-free block-2) s-return))
...
(define (test-fft) s-t
  ...
  (declare
    ...
    a-len sp-time-t)
  (set a-len 6)
  (si (sp-fft a-len a-real a-imag))
  (si (sp-ffti a-len a-real a-imag))
  (label exit s-return))

(define (test-synth) s-t
  s-declare
  (declare
    state sp-time-t*
  ...
  (s (sp-block-new channels duration &out1))
  (s (sp-block-new channels duration &out2))
  (s (sp-synth-state-new channels config-len config &state))
  (s (sp-synth out1 0 duration config-len config state))
  (s (sp-synth out2 0 duration config-len config state))
  (sp-block-free out1)
  (sp-block-free out2)
  (label exit s-return))
...
(define (test-sp-triangle-square) s-t
  s-declare
  (declare i sp-time-t out-t sp-sample-t* out-s sp-sample-t*)
  (s (sph-helper-calloc (* 96000 (sizeof sp-sample-t*)) &out-t))
  (s (sph-helper-calloc (* 96000 (sizeof sp-sample-t*)) &out-s))
...
(define (test-sp-random) s-t
  ...
  #;(for ((define i sp-time-t 0) (< i 20) (set i (+ 1 i)))
...
(define (test-sp-cheap-filter) s-t
  s-declare
  (declare
  ...
  (s (sp-cheap-filter-state-new sp-noise-duration sp-cheap-filter-passes-limit &state))
