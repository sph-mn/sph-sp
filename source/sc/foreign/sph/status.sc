...
(pre-define (status-init-group group) "like status init but sets a default group"
...
(pre-define (status-require! expression)
  ...
  (set status expression) (if status-failure? status-goto))

(pre-define (status-set-id-goto status-id) "set the status id and goto error"
  (status-set-id status-id) status-goto)
