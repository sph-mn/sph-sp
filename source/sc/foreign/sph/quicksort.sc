...
(define (quicksort less? swap element-size array array-len)
  (void (function-pointer uint8-t void* void*) (function-pointer void void* void*)
    uint8-t void* size-t)
  ...
  (declare pivot char* a char* i size-t j size-t)
  (if (< array-len 2) return)
  (set a array pivot (+ a (* element-size (/ array-len 2))) i 0 j (- array-len 1))
  (while #t
    (while (less? (+ a (* element-size i)) pivot) (set i (+ 1 i)))
    (while (less? pivot (+ a (* element-size j))) (set j (- j 1)))
    (if (>= i j) break)
