...
(define (sp-windowed-sinc-state-destroy state) (b0 sp-windowed-sinc-state-t*)
  (free (struct-pointer-get state ir))
  (free (struct-pointer-get state data))
  (free state))
...
(define (sp-windowed-sinc-state-create sample-rate freq transition state)
  ...
  (if (not (deref state))
    (begin
      ...
      (struct-pointer-set state-temp
        sample-rate 0 freq 0 transition 0 data 0 data-len 0 ir-len-prev 0))
    (set state-temp (deref state)))
  ; re-use ir, nothing changed
  (if
    (and
      (= (struct-pointer-get state-temp sample-rate) sample-rate)
  ...
  (if state
    (begin
      (free (struct-pointer-get state-temp ir))))
  (define ir-len size-t)
  (define ir sp-sample-t*)
  (sp-windowed-sinc-ir (address-of ir) (address-of ir-len) sample-rate freq transition)
  (if (not ir)
    (begin
      (if (not (deref state)) (free state-temp))
      (return 1)))
  (struct-pointer-set state-temp
    ir-len-prev (if* state (struct-pointer-get state-temp ir-len) ir-len))
    ...
    (begin
      ...
      (if (struct-pointer-get state-temp data)
        (begin
          (memcpy
            data
            (struct-pointer-get state-temp data)
  ...
  (struct-pointer-set state-temp
    ir ir ir-len ir-len sample-rate sample-rate freq freq transition transition)
  (set (deref state) state-temp)
  (return 0))
...
(define (sp-windowed-sinc result source source-len sample-rate freq transition state)
  ...
  (sp-convolve
    ...
    source
    source-len
    (struct-pointer-get (deref state) ir)
    (struct-pointer-get (deref state) ir-len)
    (struct-pointer-get (deref state) data) (struct-pointer-get (deref state) ir-len-prev))
  (return 0))
