(define-test-module (test module sph sp)
  (import (sph uniform-vector) (sph error) (sph sp) (sph list) (rnrs bytevectors))
  ...
  (define test-env-file-path "/tmp/sp-test.au") (define test-env-segment-size 8)
  ...
  (define samples-per-second 8000) (define sample-duration (/ 1 samples-per-second))
  (define segment-size (/ samples-per-second 10)) (define channels 2)
  (define output-path test-env-file-path) (define volume 0.1)
  (define (set-volume data) (map (l (e) (f32vector-map+one * volume e)) data)) (define duration 8)
