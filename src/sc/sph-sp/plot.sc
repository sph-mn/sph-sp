...
(define (sp-plot-samples->file a a-size path) (void sp-sample-t* sp-time-t char*)
  (declare file FILE* i sp-time-t)
  (set file (fopen path "w"))
  (for ((set i 0) (< i a-size) (set i (+ 1 i))) (fprintf file "%.3f\n" (array-get a i)))
  (fclose file))

(define (sp-plot-times->file a a-size path) (void sp-time-t* sp-time-t char*)
  (declare file FILE* i sp-time-t)
  (set file (fopen path "w"))
  (for ((set i 0) (< i a-size) (set+ i 1))
    (fprintf file (pre-concat-string sp-time-printf-format "\n") (array-get a i)))
  (fclose file))

(define (sp-plot-samples-file path use-steps) (void char* uint8-t)
  (declare command char* command-pattern char* command-size size-t)
  (set
...
(define (sp-plot-times-file path use-steps) (void char* uint8-t)
  (declare command char* command-pattern char* command-size size-t)
  (set
...
(define (sp-plot-samples a a-size) (void sp-sample-t* sp-time-t)
  ...
  (define path char* (calloc path-size 1))
  (if (not path) return)
  (snprintf path path-size
    (pre-concat-string "%s-" sp-time-printf-format) sp-plot-temp-path sp-plot-temp-file-index)
...
(define (sp-plot-spectrum->file a a-size path) (void sp-sample-t* sp-time-t char*)
  ...
  (for ((set i 0) (< i a-size) (set i (+ 1 i)))
    (fprintf file (pre-concat-string sp-sample-printf-format "\n")
      (* 2
        (/
          (sqrt
...
(define (sp-plot-spectrum-file path) (void char*) (sp-plot-samples-file path #t))
